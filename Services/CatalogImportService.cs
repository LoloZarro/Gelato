    ...
    ILibraryManager libraryManager)
{
        ...
        IProgress<double>? progress = null)
    {
        ...
        {
            logger.LogError("Max Collection Size reached for {CatalogName}!", catalogName);
            return;
        }
        ...
        BaseItem[] allRetrievedCatalogItems)
    {
        ...
        try
        {
            ...
            if (allRetrievedCatalogItems.Length > globalMaxItems)
            {
                logger.LogError("Max Collection Size reached for {CatalogName}!", catalogName);
                return;
            }
            ...
            await collectionManager.AddToCollectionAsync(collection.Id, allRetrievedCatalogItems.Select(i => i.Id)).ConfigureAwait(false);

            logger.LogInformation("Updated collection {CollectionName} with {Amount} new items. Total: {TotalItems}",
                catalogName, newItems.Length, allRetrievedCatalogItems.Length);
        }
        catch (Exception ex)
        {
